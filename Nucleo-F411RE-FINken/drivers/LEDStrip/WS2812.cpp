...
void WS2812::__loadBuf(int buf[],int r_offset, int g_offset, int b_offset) {
    for (int i = 0; i < __size; i++) {
        ...
        unsigned short sf; // scaling factor for  II
...
void WS2812::write_offsets (int buf[],int r_offset, int g_offset, int b_offset) {
    ...
    // Begin bit-banging
    for (int i = 0; i < FRAME_SIZE * __size; i++) {
        if (__transmitBuf[i]){
            __gpo = 1;
            for (int j = 0; j < __oneHigh; j++) {
                __NOP();
            }
            __gpo = 0;
            for (int j = 0; j < __oneLow; j++) {
                __NOP();
            }
        } else {
            __gpo = 1;
            for (int j = 0; j < __zeroHigh; j++) {
                __NOP();
            }
            __gpo = 0;
            for (int j = 0; j < __zeroLow; j++) {
                __NOP();
            }
        }
    }
