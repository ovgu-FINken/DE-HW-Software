...
#define COMP_NUMBER 4 // needs to be always equal to the current number of components
...
int main() {
    ...
    components[2] = new Sonar(uartMessenger, 0x70);
    LEDStrip *ledStrip = new LEDStrip(PC_5, 24, 0, 5, 4, 3); // experimentally defined values
    components[3] = ledStrip;
    ledStrip->setMode(1);
    //components[4] = new SimpleLED();
